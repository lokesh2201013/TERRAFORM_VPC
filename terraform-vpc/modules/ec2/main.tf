resource "google_compute_instance" "web_instances" {
  ...
  network_interface {
    network = "my-vpc"
    access_config {
      # Include this block to give the instance an external IP address
    }
  ...
  metadata_startup_script = <<-EOT
    ...
    echo "        }" >> index.html
    echo "    </style>" >> index.html
    echo "    <title>Debian Instance Status</title>" >> index.html
    echo "</head>" >> index.html
    echo "<body>" >> index.html
    echo "    <div class='wrapper'>" >> index.html
    echo "        <div class='instance-card'>" >> index.html
    echo "            <div class='instance-card__cnt'>" >> index.html
    echo "                <div class='instance-card__name'>Your Debian Instance is running!</div>" >> index.html
